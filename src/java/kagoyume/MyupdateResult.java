...
public class MyupdateResult extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try{
            ...
            //updateUdの情報をDTOにマッピング
            updateUd.UD2DTOMapping(udd);
            
            //上書き処理
            UserDataDAO.getInstance().update(udd);
            
            updateUd.DTO2UDMapping(udd);
            
            //セッションに上書き後のユーザー情報を格納
            session.setAttribute("ud", updateUd);
            session.setAttribute("udd", udd);
            session.setAttribute("user", updateUd.getName());
            
            //不要になったセッションを削除
            session.removeAttribute("updateUd");
